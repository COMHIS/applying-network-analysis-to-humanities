...
-   `left_join()` will merge the two dataframes, keeping all rows on the left, adding data from the new dataset. `inner_join` keeps only rows where the key is found in both datasets, and `anti_join()` keeps only rows where the key is *not* found in either. 
## left_join(), inner_join(), anti_join()
