...
## Steps to do well in the course {.center}
...
-   A node has a high **betweenness centrality** if it is used as a 'hop' between many paths in a network
-   **Node A** is the only way through which any of the nodes on the left can reach any on the right, which means it scores highly for **betweenness centrality**.
...
-   Each node starts off in a community of one, moved until modularity is maximised.
:::
