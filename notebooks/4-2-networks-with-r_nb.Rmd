...
For the next series of tasks, you will merge additional node attributes to the network object, and use this to filter the network. 

First, load your table of node attributes.

```{r}
nodes_df = read_csv('')
```

Next, merge this to your network object using `left_join()`. You'll need to fill in the `by = ` argument with the correct column names in the attributes dataset. The column for the node IDs in the network object is called `name`. See the class chapter for more specifics if you get stuck. 
...
Now, filter your network using an appropriate attribute *before* you calculate the relevant network statistics. Save this as a dataframe called `filtered_network_nodes`. 
...
Finally, calculate a set of groups for your network. We'll use the Louvain algorithm, with the command `group_louvain`. As above, this is added as a new column to the nodes table with `mutate`.
