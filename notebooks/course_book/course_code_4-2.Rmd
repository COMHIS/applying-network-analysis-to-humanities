---
title: "Week 4, Class 2: Network Analysis with R"
...
edge_list = letters %>% 
  group_by(from_id, to_id) %>% # make groups in your data
  tally(name = 'weight') # count these groups and name the resulting count column 'weight'

edge_list # view the edge list
...
sample_tbl_graph = edge_list %>% 
  as_tbl_graph(directed = T) # tidygraph command to create a tbl_graph from an edge list.
...
sample_tbl_graph %>% 
  activate(edges) # the activate() command switches the active table from nodes to edges
...
```{r}
sample_tbl_graph %>% 
  activate(edges) %>% # first activate the edges table
  filter(weight>1) # now the filter command will filter the edges with a weight of greater than 1
```
...
sample_tbl_graph %>% 
  ...
  mutate(degree = centrality_degree(mode = 'all', weights = weight)) # create a new column, and calculate degree using the function centrality_degree()
```
...
sample_tbl_graph %>% 
  activate(nodes) %>% # make sure nodes are active
  mutate(degree = centrality_degree(mode = 'total')) %>% # calculate degree and keep it as a new column
  filter(degree >2) # filter to only include nodes with a degree of greater than 2
...
Another related measurement is the average local clustering, calculated using the following:
...
```{r message=FALSE, warning=FALSE}
sample_tbl_graph_u %>% 
  mutate(group = group_louvain())%>%  # run community detection
  activate(nodes) %>% # make sure correct table is active
  mutate(degree = centrality_degree(weights = n, mode = 'all')) %>% # calculate degree
  mutate(between = centrality_betweenness(weights = n,directed = F)) %>%  # calculate betweenness centrality
  as_tibble()  %>% 
  group_by(group) %>% # group by the 'group' label you calculated above
  summarise(mean_degree = mean(degree,na.rm = TRUE), mean_between = mean(between, na.rm = TRUE), nodes = n()) %>% # get some summary statistics for each group %>% 
pivot_longer(2:4) %>% 
  ggplot() + # pass the code to ggplot
  geom_col(aes(x = group, y = value)) + # add a column geom
  facet_wrap(~name, scales = 'free') # facet the plot by each group
